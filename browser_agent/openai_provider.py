...
_SYSTEM_MESSAGE = (
    ...
    "Use ordinary ask_user only for genuinely missing non-secret information; "
    "use request_secret for credentials or OTP values. A username, password, PIN, "
    "or OTP remains protected even when visible on the page. Never repeat credential "
    "values in ordinary messages or completion summaries, and never put a credential "
    "bundle directly in browser_fill_form arguments. Request both username and "
    "password through request_secret; the application applies them locally without "
    "putting their values in model context. request_secret arguments "
    ...
    "and applies those values locally. Browser-action authorization is controlled "
